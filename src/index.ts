...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Chess.com');
}

const BASE_URL = 'https://api.chess.com/pub';

/**
 * Chess.com sits behind Cloudflare and 403s a request with NO User-Agent.
 *
 * This pack sent no headers at all on any of its four calls, and the Workers
 * runtime does not supply a default — so every call returned 403 and the caller
 * saw `Chess.com error: 403`, which reads as "they closed their API". They have
 * not; their API is keyless and open.
 *
 * Measured 2026-08-28 against api.chess.com/pub/player/hikaru, varying ONLY the
 * header:
 *   User-Agent: ''                                   -> 403 (Cloudflare HTML)
 *   User-Agent: curl/8.0                             -> 200
 *   User-Agent: pipeworx-mcp-chess/1.0 (+pipeworx.io) -> 200
 *   (undici's own default)                           -> 200
 * So the bar is low — it just has to be non-empty. We were sending nothing.
 *
 * Worth noting next to this: `mcps/lichess` sets a User-Agent on every fetch and
 * has never had this problem. Same repo, same week, two chess packs, one header.
 * Second occurrence tonight — phishtank was the other (fleet #574).
 */
const UA = 'pipeworx-mcp-chess/1.0 (+https://pipeworx.io)';
const HEADERS = { Accept: 'application/json', 'User-Agent': UA };

/**
 * Turn Cloudflare's challenge into a sentence, if it ever comes back.
 *
 * The tell is an HTML body where JSON belongs — a bare status told the caller
 * nothing and, being a bare 403, classified as `upstream_throttled`, the class
 * the fleet's triage deliberately skips. That is how this sat for four months
 * (github.com/pipeworx-io/mcp-chess#1, opened 2026-04-18, zero comments).
 */
async function assertNotChallenged(res: Response): Promise<void> {
  if (res.status !== 403) return;
  const body = await res.clone().text().catch(() => '');
  if (/just a moment|cf-browser-verification|challenge-platform|<!DOCTYPE html/i.test(body)) {
    throw new Error(
      'upstream_down: Chess.com answered with a Cloudflare bot challenge rather than data. Their public API is keyless and open, so this is a block on the caller — not a rate limit, not a missing key, and nothing to retry.',
    );
  }
}
...
async function getPlayer(username: string) {
  const res = await pwFetch(`${BASE_URL}/player/${encodeURIComponent(username.toLowerCase())}`, { headers: HEADERS });
  if (!res.ok) { await assertNotChallenged(res); throw await httpError(res, 'Chess.com error'); }
...
async function getStats(username: string) {
  const res = await pwFetch(`${BASE_URL}/player/${encodeURIComponent(username.toLowerCase())}/stats`, { headers: HEADERS });
  if (!res.ok) { await assertNotChallenged(res); throw await httpError(res, 'Chess.com error'); }
...
async function getLeaderboards() {
  const res = await pwFetch(`${BASE_URL}/leaderboards`, { headers: HEADERS });
  if (!res.ok) { await assertNotChallenged(res); throw await httpError(res, 'Chess.com error'); }
